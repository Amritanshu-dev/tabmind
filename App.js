...
function save() {
  localStorage.setItem('tabmind_tabs', JSON.stringify(tabs));
  updateStats();
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Load tabs
  tabs = JSON.parse(localStorage.getItem('tabmind_tabs') || '[]');
  renderTabs();
  updateStats();

  // ── AUTO-FILL URL from query parameter ──
  // The bookmarklet opens: yourdomain.com/tabmind/?url=https://...
  const params  = new URLSearchParams(window.location.search);
  const autoUrl = params.get('url');
  if (autoUrl) {
    g('input-url').value = autoUrl;
    g('input-url').style.borderColor = 'var(--accent)';
    g('bookmarklet-hint').style.display = 'none';
    // Focus the reason field since URL is already filled
    setTimeout(() => g('input-reason').focus(), 100);
  }

  // ── BUILD BOOKMARKLET LINK ──
  // Points to this page with ?url= filled from the current tab
  const siteUrl = window.location.origin + window.location.pathname;
  const bookmarkletCode = `javascript:(function(){window.open('${siteUrl}?url='+encodeURIComponent(window.location.href),'_blank');})();`;
  g('bookmarklet-link').href = bookmarkletCode;

  // ── PASTE BUTTON ──
  g('paste-btn').addEventListener('click', async () => {
    try {
      const text = await navigator.clipboard.readText();
      if (text && text.startsWith('http')) {
        g('input-url').value = text;
        g('input-url').style.borderColor = 'var(--accent)';
        g('input-url').focus();
        g('bookmarklet-hint').style.display = 'none';
        setTimeout(() => { g('input-url').style.borderColor = ''; }, 1500);
      } else {
        // Not a URL — paste anyway and let user fix it
        g('input-url').value = text;
        g('input-url').focus();
      }
    } catch (e) {
  ...
  tabs.forEach(t => {
    if (t.remindAt && t.remindAt > Date.now() && !t.done) scheduleReminder(t);
  });
...
// ─── NOTIFICATIONS ────────────────────────────────────────
function scheduleReminder(tab) {
  const delay = tab.remindAt - Date.now();
  if (delay <= 0) return;
  setTimeout(() => {
    if (Notification.permission === 'granted') {
      const du = tab.url.replace(/^https?:\/\//,'').replace(/^www\./,'');
      new Notification('⏰ TabMind Reminder', {
        body: `"${tab.reason}"\n${du}`
      });
    }
  }, delay);
...
function buildCalendar() {
  ...
  g('cal-grid').querySelectorAll('.cal-day:not(.cal-past):not(.cal-empty)').forEach(cell => {
    cell.addEventListener('click', () => {
      ...
      setTimeout(() => {
        hide('cal-popup'); clockMode = 'hour'; drawClock(); updateClockLabel(); show('clock-popup');
      }, 150);
    });
  });
}
...
function adjustHour(d) {
  selHour+=d; if(selHour>12)selHour=1; if(selHour<1)selHour=12;
  clockMode='hour'; drawClock(); updateClockLabel();
}
function adjustMin(d) {
  selMin+=d; if(selMin>=60)selMin=0; if(selMin<0)selMin=59;
  clockMode='min'; drawClock(); updateClockLabel();
}
...
function confirmReminder() {
  ...
  if (dt <= new Date()) {
    g('clock-selected-label').textContent = '⚠ This time is in the past!';
    g('clock-selected-label').style.color = '#f05a5a';
    return;
  }
  finalTimestamp = dt.getTime();
  const months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  g('remind-display-text').textContent =
    `⏰ ${months[calMonth]} ${selDay} · ${String(selHour).padStart(2,'0')}:${String(selMin).padStart(2,'0')} ${selAmPm}`;
...
function addTab() {
  ...
  const tab = {
    id: genId(), url, reason, category, customLabel,
    priority, remindAt, createdAt: Date.now(), done: false
  };

  tabs.unshift(tab);
  save();
  if (remindAt) scheduleReminder(tab);
  ...
  g('bookmarklet-hint').style.display = 'block';

  // Clear URL param from address bar without reload
...
function flash(id) {
  ...
  e.style.borderColor = '#f05a5a'; e.focus();
...
function renderTabs() {
  ...
  if (filtered.length === 0) {
    list.innerHTML = `
      <div class="empty-state">
        ...
        <div class="empty-title">${activeFilter==='all' ? 'No tabs saved yet' : 'Nothing here'}</div>
        <div class="empty-sub">${activeFilter==='all'
          ? 'Paste a URL or use the bookmarklet to auto-fill.'
  ...
  list.querySelectorAll('.done-btn').forEach(b =>
    b.addEventListener('click', () => toggleDone(b.dataset.id)));
  list.querySelectorAll('.del-btn').forEach(b =>
    b.addEventListener('click', () => deleteTab(b.dataset.id)));
...
function cardHTML(t, now) {
  ...
  let catIcon  = bi[t.category] || '📎';
  let catLabel = bl[t.category] || (t.customLabel || 'Other');
  if (t.category==='other' && t.customLabel) { catIcon='📎'; catLabel=t.customLabel; }
  const du = t.url.replace(/^https?:\/\//,'').replace(/^www\./,'');
  return `
    <div class="tab-card ${t.done?'done':''}" data-priority="${t.priority}">
      <div class="tab-main">
        <div class="tab-top">
          <span class="tab-category cat-${t.category==='other'?'other':t.category}">
            ${catIcon} ${esc(catLabel)}
          </span>
          <span class="priority-dot dot-${t.priority}"></span>
        </div>
        <a class="tab-url" href="${esc(t.url)}" target="_blank">${esc(du)}</a>
        <div class="tab-reason">${esc(t.reason)}</div>
        <div class="tab-meta">
          <span class="meta-item">🕓 ${age(t.createdAt, now)}</span>
          ${remindBadge(t.remindAt, now)}
        </div>
      </div>
...
function toggleDone(id) {
  const t=tabs.find(t=>t.id===id); if(!t)return; t.done=!t.done; save(); renderTabs();
}
...
function updateStats() {
  ...
  g('stat-due').textContent  = tabs.filter(t=>
    !t.done && t.remindAt && (
      (t.remindAt>=s.getTime() && t.remindAt<=e.getTime()) || t.remindAt<now
    )).length;
}
...
function dur(ms) {
  return ms<3600000 ? Math.round(ms/60000)+' min' : Math.round(ms/3600000*10)/10+'h';
}
function esc(s) {
  return String(s)
    .replace(/&/g,'&amp;').replace(/</g,'&lt;')
